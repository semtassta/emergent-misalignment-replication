...
- Для повторения эксперимента с открытой моделью (Qwen25-Coder-3B-Instruct) запустите файл open models experiment.ipynb. В результате выполнения кода ваша дообученная модель будет залита на Hugging Face.
- Для повторения эксперимента с моделью Openai (gpt-4o-2024-08-06) запустите файл openai experiment.ipynb. Чтобы воспользоваться готовой моделью, не производя fine-tuning, добавьте в последнюю ячейку с кодом id модели и ваш запрос. Укажите ваш OPENAI_API_KEY. Запустите ячейку с импортами и последнюю ячейку.
